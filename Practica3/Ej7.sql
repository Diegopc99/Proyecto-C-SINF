...
BEGIN
    ...
    DECLARE numero_ejecuciones INT DEFAULT 0;
    ...
    SELECT count(Directores.ID_director) INTO @num_director FROM Directores;

    SET @fecha = CURRENT_TIMESTAMP();

    IF(@numero_ejecuciones IS NULL) THEN
        SET @numero_ejecuciones = 0;
    END IF;

    SET @numero_ejecuciones = @numero_ejecuciones+1; 

    IF((SELECT count(cuentaDirectores.id_cuenta) FROM cuentaDirectores) >= 10) THEN
        DELETE FROM cuentaDirectores ORDER BY fecha limit 1;
    END IF;

    INSERT INTO cuentaDirectores(num_directores,fecha) VALUES (@num_director,@fecha);
